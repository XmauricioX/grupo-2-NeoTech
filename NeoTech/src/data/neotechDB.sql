...
CREATE TABLE `users` (
  ...
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  `user_rol_id` int(11) unsigned NOT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `users_un` (`email`),
  KEY `fkIdx_132` (`user_rol_id`),
  CONSTRAINT `FK_130` FOREIGN KEY (`user_rol_id`) REFERENCES `users_rol` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
